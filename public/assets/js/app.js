// console.log("menu");
function dropdown() {
    console.log("dropdown init ulang");

    const dropdownElements = document.querySelectorAll(".dropdown");
    console.log(dropdownElements);
    dropdownElements.forEach((dropdownToggleEl) => {
        // Cek apakah sudah ada instance sebelumnya
        let existing = bootstrap.Dropdown.getInstance(dropdownToggleEl);
        if (existing) {
            existing.dispose(); // Buang instance lama
        }
        new bootstrap.Dropdown(dropdownToggleEl); // Buat ulang
    });
}
    ...
    try {
        ...
        const h = document.querySelector(".startbar-overlay"),
            changeSidebarSize =
                ...
                () => {
                    310 <= window.innerWidth && window.innerWidth <= 1366
                        ? document.body.setAttribute(
                              "data-sidebar-size",
                              "collapsed"
                          )
